...
<!--#include file="inc/alipay_config.asp"-->
<!--#include file="inc/alipay_service.asp"-->
<%
...
If total_fee<1 Then 
response.write "<script>alert('充值金额不能低于1元,请返回重新选择.');history.back()</script>"
  response.end
End If 

'If Request.ServerVariables("HTTP_REFERER")="" Then 
'  response.write "<script language=javascript>alert('系统错误!');history.back()</script>"
'  response.end
'end If

If code="" Then
  response.write "<script language=javascript>alert('请填写验证码!');history.back()</script>"
  response.End
Else
code=Int(code)
end If

if Int(Session("getCode"))<>code Then
...
Else
'修改代码,写入数据库订单日志
ServerName=Replace(ServerName,"挑战","")
set rs=conn.execute("INSERT INTO [PayLog](UserID,UserName,Amount,trade_status,PayNo,Paytime,ServerID,Payip,Remark) VALUES('"&session("user_no")&"','"&session("UserName")&"',"&total_fee&",0,'"&out_trade_no&"','"&Now()&"','"&ServerName&"','"&Request.serverVariables("REMOTE_ADDR")&"','"&subject&body&"')")
End If 
...
<body>
        <table align="center" width="350" cellpadding="5" cellspacing="0">
            ...
            <tr>
                <td class="font_content" align="right">用户名：</td>
                <td class="font_content" align="left"><%=session("username")%></td>
            </tr>
            <tr>
